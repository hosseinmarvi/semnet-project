...
stc_SD_file_name=os.path.expanduser('~') +'/my_semnet/stc_pli_SD.json'
stc_LD_file_name=os.path.expanduser('~') +'/my_semnet/stc_pli_LD.json'
...
for k in np.arange(0,1):
    for w in np.arange(0,1):
        ...
        # brain.save_image(C.pictures_path_Source_estimate+'PLI_SD-'+C.ROIs_lables[k]+w_label[w][2:-3]+'.png')
        
        # print('Coherence_LD : '+C.ROIs_lables[k]+'-'+w_label[w])

        # brain = stc_T_LD.plot(surface='inflated', hemi='split',
        #           time_label='PLI_LD : '+C.ROIs_lables[k]+'-'+w_label[w],
        #           subjects_dir=C.data_path,size=([800,400]),
        #           clim=dict(kind='value', lims=(vmin1,(vmin1+vmax1)/2,vmax1)))
        ...
        # brain.save_image(C.pictures_path_Source_estimate+'PLI_LD-'+C.ROIs_lables[k]+w_label[w][2:-3]+'.png')

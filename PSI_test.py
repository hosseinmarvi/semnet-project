...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 12 19:00:36 2020
...
stc_total_SD=[[[[0]*2 for f in range(3)] for k in range(6)] for i in range(18)]
stc_total_LD=[[[[0]*2 for f in range(3)] for k in range(6)] for i in range(18)]
    ...
    for win in np.arange(0, len(C.con_time_window)):
        ...
        for k in np.arange(0,6):
            ...
            for f in np.arange(0,len(C.con_freq_band_psi)-1):
                ...
                stc_total_LD[i][k][f][win]= morph_LD.apply(psi_stc_LD)
end=time.time()
print(end-start)

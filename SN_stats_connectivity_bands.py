...
for i in np.arange(0,len(C.subjects)):
    ...
    # stc_SD_file_name=os.path.expanduser('~') +'/my_semnet/json_files/connectivity/stc_'+method+'200_mean_bands_SD_sub'+str(i)+'.json'
    # stc_LD_file_name=os.path.expanduser('~') +'/my_semnet/json_files/connectivity/stc_'+method+'200_mean_bands_LD_sub'+str(i)+'.json'
    # stc_blSD_file_name=os.path.expanduser('~') +'/my_semnet/json_files/connectivity/con_labels_'+method+'_bl_bands_SD_sub'+str(i)+'.json'
...
#                       time_label=method+'_SD-LD: '+C.ROIs_lables[k]+'_'+w_label[w]+'_'+f_label[f],
    ...
    for k in np.arange(4,5):
      # for k in np.array([0,4]):
        ...
        for f in np.arange(0,1):
            ...
            tstep = my_stc_coh_blLD[i][k][f].tstep
            ...
            if len(np.where(cluster_p_values<p)[0])!=0:           
                ...
                brain = stc_all_cluster_vis.plot(surface='inflated', hemi='split',subject =\
                    ...
                    [thresh/4,thresh/2,thresh]),size=(800,400),colormap='mne',time_label=\
                      'BL (F-LD): '+C.ROIs_lables[k]+'_'+w_label[w]+'_'+f_label[f],views='lateral')

...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 13 07:24:49 2020
...
stc_SD_file_name=os.path.expanduser('~') +'/my_semnet/stc_plv_SD.json'
stc_LD_file_name=os.path.expanduser('~') +'/my_semnet/stc_plv_LD.json'
...
#         for i in np.arange(0,18):
        ...
        #           time_label='PLV_SD : '+C.ROIs_lables[k]+'-'+w_label[w],
        #           subjects_dir=C.data_path,size=([800,400]),
        #           clim=dict(kind='value', lims=(vmin1,(vmin1+vmax1)/2,vmax1)))
     
        # brain.save_image(C.pictures_path_Source_estimate+'PLV_SD-'+C.ROIs_lables[k]+w_label[w][2:-3]+'.png')
        
        # print('Coherence_LD : '+C.ROIs_lables[k]+'-'+w_label[w])

        # brain = stc_T_LD.plot(surface='inflated', hemi='split',
        #           time_label='PLV_LD : '+C.ROIs_lables[k]+'-'+w_label[w],
        #           subjects_dir=C.data_path,size=([800,400]),
        #           clim=dict(kind='value', lims=(vmin1,(vmin1+vmax1)/2,vmax1)))
        # brain.save_image(C.pictures_path_Source_estimate+'PLV_LD-'+C.ROIs_lables[k]+w_label[w][2:-3]+'.png')
...
for k in np.arange(3,4):
    ...
    for i in np.arange(0,18):
        a=(np.transpose(my_stc_coh_SD[i][k][0].data, [1, 0])+\
            np.transpose(my_stc_coh_SD[i][k][1].data, [1, 0]))/2
        b=(np.transpose(my_stc_coh_LD[i][k][0].data, [1, 0])+\
            np.transpose(my_stc_coh_LD[i][k][1].data, [1, 0]))/2
    ...
    tstep = my_stc_coh_SD[i][k][0].tstep
    n_vertices_sample, n_times = my_stc_coh_SD[i][k][0].data.shape
    fsave_vertices = [np.arange(10242), np.arange(10242)]
    ...
    brain.save_image(C.pictures_path_Source_estimate+'Connectivity_PLV_'+\
                      C.ROIs_lables[k]+'.png')

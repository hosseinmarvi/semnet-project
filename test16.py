...
#                       time_label='Coh_SD: '+C.ROIs_lables[k]+'_'+w_label[w]+'_'+f_label[f],
#                       subjects_dir=C.data_path,size=([800,400]),
#                       clim=dict(kind='value', lims=(v_min,vmid,v_max)))
#             brain.save_image(C.pictures_path_Source_estimate+'Coh_SD-'+C.ROIs_lables[k]+'_'+w_label[w][:-3]+'_'+f_label[f]+'.png')
            
#             print('Coherence_LD : '+C.ROIs_lables[k]+'-'+w_label[w]+'-'+f_label[f])
    
#             brain = stc_T_LD.plot(surface='inflated', hemi='split',
#                       time_label='Cohe_LD : '+C.ROIs_lables[k]+'_'+w_label[w]+'_'+f_label[f],
#                       subjects_dir=C.data_path,size=([800,400]),
#                       clim=dict(kind='value', lims=(v_min,vmid,v_max)))
#             brain.save_image(C.pictures_path_Source_estimate+'Coh_LD-'+C.ROIs_lables[k]+'_'+w_label[w][:-3]+'_'+f_label[f]+'.png')
    ...
    for k in np.arange(0,6):
        for f in np.arange(0,4):
            ...
            if len(np.where(cluster_p_values<0.05)[0])!=0:           
                ...
                brain.save_image(C.pictures_path_Source_estimate +'t-test_coh_' + \

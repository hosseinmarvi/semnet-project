...
MEG_bad_channels_fruit = dict()
MEG_bad_channels_odour = dict()
MEG_bad_channels_milk  = dict()
MEG_bad_channels_LD    = dict()
...
SD_categories=['visual','hear','hand','emotional','neutral','pwordc','target']
LD_categories=['visual','hear','hand','emotional','neutral','pwordc','pworda','filler']
fname_SD = {'fruit':'block_fruit_epochs-epo.fif','odour':'block_odour_epochs-epo.fif',
            'milk':'block_milk_epochs-epo.fif','SD_words':'block_SD_words_epochs-epo.fif'}

fname_LD = {'LD':'block_LD_epochs-epo.fif','LD_words':'block_LD_words_epochs-epo.fif'}
...
pictures_path_Source_estimate = os.path.expanduser('~') + '/Python/pictures/Source_Signals/'
# pictures_path = os.path.expanduser('~') + '/Python/pictures/Source_Signals/'

pictures_path_evoked_white = os.path.expanduser('~') + '/Python/pictures/evoked_white/'
pictures_path_cove = os.path.expanduser('~') + '/Python/pictures/plot_cov/'
pictures_path_grand_average = os.path.expanduser('~') + '/Python/pictures/SD-LD_task_pictures/'
...
categories_SD=['visual','hear','hand','emotional','neutral','pwordc','target','words']
categories_LD=['visual','hear','hand','emotional','neutral','pwordc','pworda','filler','words']


###########################################################
# stc.plot / evoked distributation, t-maps, Cluster_based permutation test
###########################################################
...
sfreq = 1000
ImCoh_SD = np.zeros([len(subjects),len(con_freq_band)-1,len(con_time_window),6,6])
ImCoh_LD = np.zeros([len(subjects),len(con_freq_band)-1,len(con_time_window),6,6])
ImCoh_SD_LD = np.zeros([len(subjects),len(con_freq_band)-1,len(con_time_window),6,6])
ImCoh_SD_sorted = np.zeros([len(con_time_window),len(con_freq_band)-1,6,6])
ImCoh_LD_sorted = np.zeros([len(con_time_window),len(con_freq_band)-1,6,6])



## Timeseries: ROIs
epochs_names =['block_SD_words_epochs-epo.fif','block_LD_words_epochs-epo.fif']
inv_op_name = ['InvOp_SD_EMEG-inv.fif','InvOp_LD_EMEG-inv.fif']
pvalue=0.05
ROIs_lables = ['lATL','rATL','PTC','IFG','AG','PVA']
n_permutations=5000
